...
const DtNode: React.FC<DeNodeProps> = ({ data }) => {
  return (
    <Flex align="center" gap="small" style={flexStyle}>
      <LibraryFolder />
      <Typography.Text>{data.data.label}</Typography.Text>
    </Flex>
      ...
      const graph = new Graph({
        ...
        node: {
          ...
          style: {
            // size: [60, 20],
            size(d: any) {
              const width = d.data.label.length * 14 + 30; // textNum * 20px
              console.log("d", d.data.label.length, "width", width);
              const height = 30;
              return [width, height];
            },
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <button title="还原" onClick={handleZoomTo}>1:1</button>
